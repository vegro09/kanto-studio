...
import { Trash2, Music, Volume2, FastForward, Activity, Sparkles } from 'lucide-react';
  ...
  if (!selectedClip || (selectedClip.type && selectedClip.type !== 'audio')) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-6 text-neutral-500">
        ...
        <p className="text-[11px] text-neutral-600 mt-1">Select an audio clip on the timeline (SFX 1 or Voice Over) to tweak its DSP properties.</p>
      </div>
    );
  }
    ...
    <div className="space-y-5 select-none text-white">
      ...
      <div className="p-3 bg-neutral-900 rounded-lg border border-neutral-800 flex justify-between items-center shadow-sm">
        <div className="truncate pr-2">
          ...
          <span className="text-[10px] text-[#00E5FF] font-mono">{duration.toFixed(2)}s Duration</span>
        </div>
        {removeClip && (
          <button
      ...
      <div className="space-y-3">
        <span className="text-[11px] font-bold tracking-wider text-neutral-400 uppercase flex items-center gap-1.5">
          <Volume2 className="w-3.5 h-3.5 text-[#00E5FF]" /> Levels & Pitch
        </span>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            className="w-full h-1.5 bg-neutral-800 rounded-lg appearance-none accent-[#00E5FF] cursor-pointer"
          />
        </div>
      ...
      <div className="space-y-2 pt-3 border-t border-neutral-800">
        <span className="text-[11px] font-bold tracking-wider text-neutral-400 uppercase flex items-center gap-1.5">
          <Sparkles className="w-3.5 h-3.5 text-[#00E5FF]" /> Voice & Audio DSP Filter
        </span>
        <select
          value={selectedClip.filterId || 'none'}
          onChange={(e) => updateClip && updateClip(selectedClip.id, { filterId: e.target.value })}
          className="w-full bg-neutral-900 border border-neutral-800 rounded-lg p-2 text-xs text-white focus:outline-none focus:border-[#00E5FF]/50 cursor-pointer"
        >
      ...
      <div className="space-y-3 pt-3 border-t border-neutral-800">
        <span className="text-[11px] font-bold tracking-wider text-neutral-400 uppercase flex items-center gap-1.5">
          <Activity className="w-3.5 h-3.5 text-[#00E5FF]" /> Attack & Fades
        </span>
